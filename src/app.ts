...
export const RunPing = async(isCron:boolean) => {
    ...
    for (let host of PING_CONFIG.hosts) {
        ...
        if (isCron) setPingHistory(host, ip, alive);
    }
    return result;
}
...
export const RunFetch = async(isCron:boolean) => {
    ...
    for (let url of FETCH_CONFIG.url) {
        ...
        if (isCron)  setFetchHistory(url, status, alive);
    }
    return result;
}

const setPingHistory = (host:string, ip:string, alive:boolean) => {
...
const setFetchHistory = (url:string, status:string, alive:boolean) => {
